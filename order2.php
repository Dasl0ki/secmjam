...
if(isset($_SESSION["user"])) {
    ...
    foreach($food as $foodid) {
        ...
        $fullprice = $fullprice + getPrice($foodid, $amount);
    }
    
    if(checkAfford($fullprice, $user) == TRUE) {
        foreach ($food as $foodid) {
            $foodid_amount = $foodid."_amount";
            $amount = filter_input(INPUT_POST, $foodid_amount, FILTER_SANITIZE_SPECIAL_CHARS);
            for($i=1; $i <= $amount; $i++) {
        ...
        changeBalance($user, $fullprice, 'sub');

        echo "Bestellung erfolgreich gespeichert";               
    } else {
        echo 'Zu wenig Kohle<br><br><a href="menue.php">Zurück</a>';
    }
